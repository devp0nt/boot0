...
```sh
git clone https://github.com/devp0nt/boot0.git
cd boot0
bun install
```
...
```
feat: add isServiceStarted helper
fix: stop services in reverse dependency order
docs: clarify the hidden option
```

Mark a breaking change with `!`:

```
feat!: rename createRuntime() options
```
